...
class YahooCollectorCN(YahooCollector, ABC):
    def get_instrument_list(self):
        logger.info("Reading symbols from local file: ./SMCI.txt")
        with open("SMCI.txt", "r") as f:
...
class YahooCollectorUS(YahooCollector, ABC):
    def get_instrument_list(self):
        logger.info("Reading symbols from local file: ./nasdaq_symbols.txt")
        with open("nasdaq_symbols.txt", "r") as f:
            symbols = [line.strip() for line in f.readlines() if line.strip()]
        logger.info(f"Loaded {len(symbols)} symbols.")
        return symbols
        # logger.info("get US stock symbols......") 获取所有美股
...
class YahooNormalize(BaseNormalize):
    ...
    def normalize_yahoo(
        ...
        if calendar_list is not None:
            calendar_list = pd.to_datetime(calendar_list, utc=True).tz_convert(None)
            calendar_index = pd.DatetimeIndex(calendar_list)
            df = df.reindex(
                calendar_index[
                    (calendar_index >= df.index.min()) &
                    (calendar_index <= df.index.max() + pd.Timedelta(hours=23, minutes=59))
                    ]
            )
            # df = df.reindex(
...
class YahooNormalize1dExtend(YahooNormalize1d):
    def __init__(
            self, old_qlib_data_dir: [str, Path], date_field_name: str = "date", symbol_field_name: str = "symbol",
            **kwargs
    ):
        """
...
class YahooNormalize1min(YahooNormalize, ABC):
    ...
    def __init__(
            self, qlib_data_1d_dir: [str, Path], date_field_name: str = "date", symbol_field_name: str = "symbol",
            **kwargs
    ):
        """
...
class AkShareCollectorCN(BaseCollector):
    def get_instrument_list(self):
        logger.info("读取 ./SMCI.txt 中的 A 股代码")
        with open("SMCI.txt", "r") as f:
            symbols = [line.strip() for line in f.readlines() if line.strip()]
        logger.info(f"加载 {len(symbols)} 支股票")
        return symbols

    def normalize_symbol(self, symbol: str) -> str:
        # 对 A 股一般不需要转换，直接返回原始代码
        return symbol

    def get_data(self, symbol, interval, start_datetime, end_datetime) -> pd.DataFrame:
        try:
...
class Run(BaseRun):
    ...
    def collector_class_name(self):
        ...
        if self.region.upper() == "CN":
            return "AkShareCollectorCN"
        else:
            return f"YahooCollector{self.region.upper()}{self.interval}"
